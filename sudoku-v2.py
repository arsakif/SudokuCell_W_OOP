...
sdk_df_copy = sdk_df
CheckLegit(sdk_df_copy).check_legit()
...
class SudokuSolve(SudokuCell):
    ...
    def iterate_cells(self):
        ...
        if not self.no_solution_flag:  # If reached a solution write solution back to the Excel File
            ...
            return [1, self.assgmt_hstry_ls]



print(sdk_df)
assgmt_hstry_ls = [[10, 10, []]]
SudokuSolve(sdk_df_copy, assgmt_hstry_ls).iterate_cells()
print(sdk_df)
numm = [1]
...
class CheckUniqueSolution(SudokuSolve):
    def __init__(self, sdku_df):
        ...
        self.sdk = sdk_df
        print(self.sdk)
        self.assgmt_hist = SudokuSolve(self.sdk).iterate_cells()[1]
        print(self.assgmt_hist)
        self.number_of_solutions = 1

    def check_another_solution(self):
        while len(self.assgmt_hist) > 1:
            if not self.assgmt_hist[1][2]:

                del self.assgmt_hist[1]
            else:
                sdk_temp = self.sdk
                print(self.assgmt_hist)
                sdk_temp.loc[self.assgmt_hist[1][0], self.assgmt_hist[1][1]] = self.assgmt_hist[1][2].pop()
                if SudokuSolve(sdk_temp).iterate_cells():
                    self.number_of_solutions += 1
                    numm[0] = numm[0] + 1
                    ass_ls_temp = self.assgmt_hist
                    CheckUniqueSolution(sdk_temp).check_another_solution()
        print(self.number_of_solutions)


CheckUniqueSolution(sdk_df).check_another_solution()
print(numm[0])
